...
module.exports =
{
    ...
    new_tkt: function(args, bot) {
      ...
      this.redmine_post_request('/issues.json', payload, bot, function (response_body) {
        var issue_id = response_body['issue']['id']
        var url = process.env.REDMINE_PROTOCOL + '://' + process.env.REDMINE_PUBLIC_HOST + ":" + process.env.REDMINE_PUBLIC_PORT + '/issues/' + issue_id
        bot.postWithParams("*New issue got created:*\n"+"> :notebook: "+ url)
      });
    },
